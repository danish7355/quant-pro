...
const appHeader = fs.readFileSync('app_header.txt', 'utf8');
...
const newAppRender = `
  return (
    <div className="flex h-screen bg-[#09090b] text-white overflow-hidden font-sans selection:bg-indigo-500/30">
      ...
      {/* Sidebar */}
      <div className={\`fixed md:static inset-y-0 left-0 w-64 bg-[#05080f] border-r border-[#30363D] flex flex-col z-10 transition-transform duration-300 ease-in-out \${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}\`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-[#09090b] via-[#05080f] to-[#020408]">
          {activeTab === 'scanner' && (
            <ScannerList 
              coins={coins} 
              onSelectCoin={(c) => setSelectedSymbol(c.symbol)} 
...
fs.writeFileSync('src/App.tsx', appHeader + newAppLogic + newAppRender);
